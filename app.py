...
# Enhanced Brutalist Visuals (DIKEMBALIKAN)
st.markdown("""
    <style>
...
if student_file and key_file:
    ...
    # 5. TEST-LEVEL STATS (DIKEMBALIKAN)
    ...
    def apply_full_styling(row):
        ...
        styles[8] = styles[9] = f'background-color: {val_bg}; color: black'
    ...
    # 7. INTERPRETATION REPORT (DIKEMBALIKAN)
    st.divider()
    ...
    # 8. DISTRACTOR ANALYSIS (LENGKAP: FORMAT 1 (10%) + WARNA + INTERPRETASI)
    ...
    def interpret_distractor_new(row_idx):
        effective = [opt for opt in cols if dist_pct.loc[row_idx, opt] >= 0.05 and opt != "N/A"]
        return f"Effective: {', '.join(effective)}" if effective else "No effective distractors"
    
    df_dist_display['Interpretation'] = [interpret_distractor_new(idx) for idx in df_dist_display.index]

    def color_dist_v2(val, pct):
        return f'background-color: rgba(46, 204, 113, {pct})'

    st.dataframe(df_dist_display.style.apply(lambda x: [color_dist_v2(v, dist_pct.loc[x.name, col]) if col in dist_pct.columns else '' for col, v in x.items()], axis=1), use_container_width=True)

    # 9. GUIDE DATA (DIKEMBALIKAN)
    guide_data = {
    ...
    st.download_button(label="📥 Download Full Report", data=buf.getvalue(), file_name="Complete_Item_Analysis.xlsx")
